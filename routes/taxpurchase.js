   ...
   try{
    ...
    for(const item of items){
        ...
        await db.promise().query(
            "INSERT INTO purchase_entry_items (purchase_id, item_name, price, quantity, hsn, uom, amount) VALUES (?, ?, ?, ?, ?, ?, ?)",
            [purchaseId, item.item_name, item.price, item.quantity, item.hsn, item.uom, amount]
        );
    }
...
router.get("/:billno", async (req, res) => {
  try {
    ...
    const [items] = await db.promise().query(
      "SELECT item_name, price, quantity, hsn, uom FROM purchase_entry_items WHERE purchase_id = ?",
      [purchase.id]
    );
